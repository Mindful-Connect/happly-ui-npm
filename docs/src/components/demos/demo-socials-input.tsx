...
// =============================================================================
// ICONS (Inlined from source to avoid dependencies)
// =============================================================================
...
// =============================================================================
// CUSTOM INPUT IMPL (Supports leftIcon, rightAffix)
// =============================================================================
...
function generateSocialURLFromHandle(key: string, text: string) {
  ...
  switch (key) {
    ...
    case 'youtube':
      ...
      break
    case 'instagram':
      if (text[0] === '@') username = text.substring(1)
      if (username) url = `https://instagram.com/${username}`
      break
    case 'facebook':
      if (text[0] === '@') username = text.substring(1)
      if (username) url = `https://facebook.com/${username}`
      break
    case 'x-twitter':
      if (text[0] === '@') username = text.substring(1)
      if (username) url = `https://x.com/${username}`
      break
    case 'linkedin':
      if (text[0] === '@') username = text.substring(1)
      if (username) url = `https://linkedin.com/in/${username}`
      break
    case 'zoom':
    case 'calendar':
    default:
...
function SocialsInputsSelectableDS({
  name,
  readOnly = false,
  availableSocials = [
    'linkedin',
  ...
  // Refactored validation without Zod
  function checkUrlValidity(key: string, url: string, message: string) {
  ...
  function handleCheckSocialUrl({ key, url }: { key: string; url: string }) {
    ...
    // Exception for calendar type which might just be any URL or specific check?
    // Original code: if (!result.success && !((key === 'calendar'))) ...
    // So calendar validation is looser?
    // Actually original code says: if (!result.success && !((key === 'calendar')))
    // This implies calendar fails Zod but we ignore it? Or we don't run Zod for calendar?
    // Let's assume validation is required for others.

    let isValid = false
    if (key === 'calendar') {
      isValid = validateURL(url) // Just format check
    } else {
      const check = checkUrlValidity(key, url, message)
      isValid = check.success
    }

    if (!isValid) {
      const newErrors = {} as Record<string, string | undefined>
      newErrors[`${name}.${key}`] = message // Simplified error mapping
  ...
  const handleInputBlur = async ({
    ...
    if (!value || value.includes(' ')) {
      setErrorTimeout(
        window.setTimeout(() => {
          setErrors({ ...errors, [`${name}.${key}`]: t('errorSocialUrl') })
        }, 80),
      )
      return
    }

    let message = t('errorSocialUrl')
    if (key === 'calendar') message = t('errorCalendarUrl')
    else if (key === 'zoom') message = t('errorZoomUrl')

    const validateURLResult = validateURL(value)

    // Initial regex check
    if (key === 'calendar' && !validateURLResult) {
      setErrorTimeout(
        window.setTimeout(() => {
          setErrors({ ...errors, [`${name}.${key}`]: message })
        }, 80),
      )
      return
    }
    ...
    const result = handleCheckSocialUrl({ key, url })
    if (result) {
  ...
  function getInputPlaceholder(key: string) {
    if (key === 'linkedin') return 'https://linkedin.com/in/'
    if (key === 'instagram') return 'https://instagram.com'
    if (key === 'facebook') return 'https://facebook.com'
    if (key === 'youtube') return 'https://youtube.com'
    if (key === 'x-twitter') return 'https://x.com'
    if (key === 'tiktok') return 'https://tiktok.com'
    if (key === 'zoom') return 'https://zoom.us'
    return 'https://socialmedia.com'
  }

  // Wrapper for icons to ensure size
  const IconWrapper = ({ children }: { children: React.ReactNode }) => (
    <div className="h-4 w-4 [&>svg]:h-full [&>svg]:w-full">{children}</div>
  )
                  ...
                  {socials.map((social, index) => {
                    return (
                      <div
                        ...
                        className="flex cursor-pointer items-center gap-x-[18px] p-2 shadow-[0px_4.548px_4.548px_0px_rgba(0,0,0,0.01)] hover:bg-black/5"
                      >
              ...
              return (
                <div className="relative flex flex-col gap-y-1.5" key={key}>
                  <Input
                    ...
                    onPaste={async (e) => {
                      handleInputPaste({ e, key })
                    }}
                    onFocus={() => setFocusedInput(key)}
                    onBlur={async (e) => {
                      if (focusedInput === key) setFocusedInput(null)
                      handleInputBlur({ e, key, value: e.currentTarget.value })
                    }}
                    onChange={(e) =>
...
export function DemoSocialsInput() {
  const [values, setValues] = useState<Record<string, string>>({
    linkedin: 'https://linkedin.com/in/happly',
  })

  const [valuesFr, setValuesFr] = useState<Record<string, string>>({})

  return (
    <div className="flex w-full max-w-xl flex-col gap-10 p-6">
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Default Labels (English)</h3>
        <div className="bg-white p-4">
          <SocialsInputsSelectableDS
            name="socials_en"
            formValue={values}
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">French Labels</h3>
        <div className="bg-white p-4">
          <SocialsInputsSelectableDS
            name="socials_fr"
            formValue={valuesFr}
            setFormValue={setValuesFr}
            labels={{
              addSocialMedia: 'Ajouter un média social',
              errorSocialUrl:
                "Veuillez saisir une URL de réseau social ou un nom d'utilisateur valide",
              errorCalendarUrl: 'Veuillez saisir une URL valide',
              errorZoomUrl: 'Veuillez saisir une URL Zoom valide',
            }}
          />
        </div>
        <div className="text-muted-foreground text-xs">
