...
const SCALE_STEPS = [950, 900, 800, 700, 600, 500, 400, 300, 200, 100, 50] as const;
    ...
    <div className="flex gap-1">
      {SCALE_STEPS.map((step) => (
        <div key={step} className="flex flex-1 flex-col items-center gap-1">
          ...
          />
          <span className="text-[10px] tabular-nums text-text-soft-400">{step}</span>
        </div>
      ))}
    </div>
  </div>
...
const SemanticRow = ({ label, tokens }: { label: string; tokens: ReadonlyArray<{ name: string; variable: string }> }) => (
  <div className="flex flex-col gap-1.5">
    <span className="text-xs font-medium text-text-sub-600">{label}</span>
    <div className="flex gap-1">
      {tokens.map((token) => (
        <div key={token.name} className="flex flex-1 flex-col items-center gap-1">
          <div
            className="h-10 w-full rounded-md ring-1 ring-black/5"
            style={{ background: `var(${token.variable})` }}
          />
          <span className="text-[10px] text-text-soft-400">{token.name}</span>
        </div>
      ))}
    </div>
  </div>
...
const FullPalette = ({ label, color }: { label: string; color: string }) => (
  <ThemeProvider.Root primaryColor={color} className="flex flex-col gap-5">
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-mono text-text-soft-400">{color}</span>
    </div>
