...
export const Lighter = {
  render: () => (
    <div className='flex flex-wrap items-end gap-4'>
      ...
      <KeyIcon.Root style='lighter' color='orange' icon={<RiFlashlightLine />} />
      ...
      <KeyIcon.Root style='lighter' color='pink' icon={<RiNotification3Line />} />
    ...
    <div className='flex flex-col gap-6'>
      {(['sm', 'md', 'lg', 'xl', '2xl'] as const).map((size) => (
        <div key={size} className='flex items-center gap-4'>
          <span className='w-8 text-paragraph-sm text-text-sub-600'>{size}</span>
          <KeyIcon.Root size={size} style='stroke' icon={<RiHome2Line />} />
          <KeyIcon.Root size={size} style='lighter' color='blue' icon={<RiHome2Line />} />
        </div>
      ))}
    </div>
  ),
...
export const Colors = {
  render: () => (
    <div className='flex flex-col gap-4'>
      {(
        ['gray', 'blue', 'orange', 'red', 'green', 'yellow', 'purple', 'pink', 'teal'] as const
      ).map((color) => (
        <div key={color} className='flex items-center gap-4'>
          <span className='w-16 text-paragraph-sm text-text-sub-600'>{color}</span>
          <KeyIcon.Root size='lg' style='stroke' color={color} icon={<RiStarLine />} />
          <KeyIcon.Root size='lg' style='lighter' color={color} icon={<RiStarLine />} />
        </div>
      ))}
    </div>
  ),
