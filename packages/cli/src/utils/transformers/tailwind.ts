...
function generateThemeBlock(): string {
  ...
  Object.entries(texts).forEach(([key, value]) => {
    ...
    if (options.lineHeight) lines.push(`  --text-${key}--line-height: ${options.lineHeight};`);
    if (options.letterSpacing) lines.push(`  --text-${key}--letter-spacing: ${options.letterSpacing};`);
    if (options.fontWeight) lines.push(`  --text-${key}--font-weight: ${options.fontWeight};`);
  });
